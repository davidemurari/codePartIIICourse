...
def get_plots(soln,t_eval,method):
    ...
    plt.plot(t_eval[0],soln[0,0],'r-',label=r"$q$")
    plt.plot(t_eval,soln[1],'k-')
    plt.plot(t_eval[0],soln[0,1],'k-',label=r"$p$")
    plt.xlabel(r"$t$")
    plt.legend()


    plt.subplot(1, 3, 2, aspect="equal")
    plt.plot(soln[0],soln[1],'k-',linewidth=1)
    
    x_circle = np.cos(t_eval)
    y_circle = np.sin(t_eval)
    plt.plot(x_circle,y_circle,'c-',linewidth=1)
    
    plt.plot(x_circle[0],y_circle[0],'c-',linewidth=1)
    plt.plot(soln[0,0],soln[1,0],'ro')
    plt.xlabel(r"$q$")
    plt.ylabel(r"$p$")
    #plt.legend()
    ...
    plt.xlabel(r"$t$")
    plt.ylabel(r"$\left|H(q(t),p(t))-H(q_0,p_0)\right|$")

    plt.suptitle(f"Results with {method}")

    plt.savefig(f"simpleHO_{method}.pdf",bbox_inches="tight")
